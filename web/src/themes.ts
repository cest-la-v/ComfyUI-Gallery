...
export type Theme = {
  name: string;
  label: string;
  /** OKLCH color string used to render the swatch circle */
  preview: string;
  vars: { light: ThemeVars; dark: ThemeVars };
};

/**
 * Full-palette base themes. Override all CSS variables — backgrounds, cards,
 * borders, and the primary accent. Source: shadcn-ui/ui registry/themes.ts.
 * When selected, the ComfyUI dark-mode integration (--color-surface-* vars) is
 * replaced by the theme's standalone dark palette.
 */
export const BASE_THEMES: Theme[] = [
  {
    name: 'zinc',
    label: 'Zinc',
    preview: 'oklch(0.705 0.015 286.067)',
    vars: {
      light: {
  ...
  },
  {
    name: 'stone',
    label: 'Stone',
    preview: 'oklch(0.709 0.01 56.259)',
    vars: {
      light: {
        background: 'oklch(1 0 0)',
  ...
  },
  {
    name: 'mauve',
    label: 'Mauve',
    preview: 'oklch(0.711 0.019 323.02)',
    vars: {
      light: {
        background: 'oklch(1 0 0)',
  ...
  },
  {
    name: 'mist',
    label: 'Mist',
    preview: 'oklch(0.723 0.014 214.4)',
    vars: {
      light: {
        background: 'oklch(1 0 0)',
  ...
  },
  {
    name: 'olive',
    label: 'Olive',
    preview: 'oklch(0.737 0.021 106.9)',
    vars: {
      light: {
        background: 'oklch(1 0 0)',
  ...
  },
  {
    name: 'taupe',
    label: 'Taupe',
    preview: 'oklch(0.714 0.014 41.2)',
    vars: {
      light: {
        background: 'oklch(1 0 0)',
...
/**
 * Accent color overlays. Source: shadcn-ui/ui registry/themes.ts
 * Stacked on top of a base theme. Overrides primary, secondary, charts, sidebar.
 */
export const ACCENT_THEMES: Theme[] = [
  {
    name: 'amber',
    label: 'Amber',
  ...
  },
  {
    name: 'cyan',
    label: 'Cyan',
    preview: 'oklch(0.52 0.105 223.128)',
    vars: {
      light: {
  ...
  },
  {
    name: 'fuchsia',
    label: 'Fuchsia',
    preview: 'oklch(0.518 0.253 323.949)',
    vars: {
      light: {
  ...
  },
  {
    name: 'indigo',
    label: 'Indigo',
    preview: 'oklch(0.457 0.24 277.023)',
    vars: {
      light: {
  ...
  },
  {
    name: 'orange',
    label: 'Orange',
    preview: 'oklch(0.553 0.195 38.402)',
    vars: {
      light: {
  ...
  },
  {
    name: 'pink',
    label: 'Pink',
    preview: 'oklch(0.525 0.223 3.958)',
    vars: {
      light: {
  ...
  },
  {
    name: 'purple',
    label: 'Purple',
    preview: 'oklch(0.496 0.265 301.924)',
    vars: {
      light: {
  ...
  },
  {
    name: 'rose',
    label: 'Rose',
    preview: 'oklch(0.514 0.222 16.935)',
    vars: {
      light: {
...
/** Build a CSS variable block string for a given vars object. */
...
/**
 * Generate a `<style>` tag content for the given base theme.
 * Returns null if name is 'default' (use existing globals.css vars).
 */
export function buildBaseThemeCss(name: string): string | null {
  const theme = BASE_THEMES.find(t => t.name === name);
  if (!theme) return null;
  const rootSel = ':is(#comfy-gallery-root, #comfy-gallery-yarl-root)';
  const darkSel = ':is(#comfy-gallery-root.dark, #comfy-gallery-yarl-root.dark)';
  return [
    buildCssBlock(rootSel, theme.vars.light),
    buildCssBlock(darkSel, theme.vars.dark),
  ].join('\n\n');
}

/**
 * Generate a `<style>` tag content for the given accent color theme.
 * Returns null if name is 'default'.
 */
export function buildAccentThemeCss(name: string): string | null {
